...
// Inspects the union of Wave 1 + Wave 2 tables (Wave 2 is additive on top of an
// already-migrated Wave 1 database; W1-F01 is expected MATERIALIZED on `files`).
$root = dirname(__DIR__);
$wave1Manifest = json_decode(file_get_contents($root.'/docs/database/physical/wave1_manifest.json'), true, 512, JSON_THROW_ON_ERROR);
$wave2Manifest = json_decode(file_get_contents($root.'/docs/database/physical/wave2_manifest.json'), true, 512, JSON_THROW_ON_ERROR);
$tableNames = [...array_map(static fn($m) => $m['table'], $wave1Manifest['migrations']), ...$wave2Manifest['tables']];
...
if (in_array('--capture', $argv, true)) {
    ...
    $migrations = [];
    foreach ($wave1Manifest['migrations'] as $entry) {
        $migration = require $root.'/apps/api/database/migrations/'.$entry['file'];
        $migration->up();
        $migrations[] = $migration;
    }
    foreach ($wave2Manifest['migrations'] as $file) {
...
try {
    ...
    if (!$dsn || !str_starts_with($dsn, 'mysql:') || !str_contains($dsn, 'dbname=')) {
        throw new RuntimeException('Explicit WAVE2_DSN with database is required');
    }
    $pdo = new PDO($dsn, getenv('WAVE2_USER') ?: '', getenv('WAVE2_PASSWORD') ?: '', [
    ...
    fwrite(STDERR, 'Wave 2 inspection blocked; error code: '.$e->getCode().'. No credentials or connection details emitted.'.PHP_EOL);
    exit(2);
}

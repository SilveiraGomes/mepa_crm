...
// Inspects Wave 3 plus its explicit catalogue-approved audit support.
$root = dirname(__DIR__);
$manifest = json_decode(file_get_contents($root.'/docs/database/physical/wave3_manifest.json'), true, 512, JSON_THROW_ON_ERROR);
...
$tableNames=[...array_map(fn($m)=>$m['table'],$w1['migrations']),...$w2['tables'],...$manifest['tables']];
...
try {
    ...
    if (!$dsn || !str_starts_with($dsn, 'mysql:') || !str_contains($dsn, 'dbname=')) {
        throw new RuntimeException('Explicit WAVE3_DSN with database is required');
    }
    $pdo = new PDO($dsn, getenv('WAVE3_USER') ?: '', getenv('WAVE3_PASSWORD') ?: '', [
    ...
    fwrite(STDERR, 'Wave 3 inspection blocked; error code: '.$e->getCode().'. No credentials or connection details emitted.'.PHP_EOL);
    exit(2);
}

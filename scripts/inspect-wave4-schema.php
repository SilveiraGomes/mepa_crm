...
// Inspects Wave 4 plus its explicit catalogue-approved audit support.
$root = dirname(__DIR__);
$manifest = json_decode(file_get_contents($root.'/docs/database/physical/wave4_manifest.json'), true, 512, JSON_THROW_ON_ERROR);
$w1=json_decode(file_get_contents($root.'/docs/database/physical/wave1_manifest.json'),true);
$w2=json_decode(file_get_contents($root.'/docs/database/physical/wave2_manifest.json'),true);
$w3=json_decode(file_get_contents($root.'/docs/database/physical/wave3_manifest.json'),true);
$tableNames=[...array_map(fn($m)=>$m['table'],$w1['migrations']),...$w2['tables'],...$w3['tables'],...$manifest['tables']];
if (in_array('--capture', $argv, true)) {
    ...
    foreach ($manifest['migrations'] as $file) {
        $migration=require $root.'/apps/api/database/migrations/'.$file;
        $migration->up(); $migrations[]=$migration;
    }
    echo json_encode(['mode'=>'STATIC_CAPTURE_NOT_EXECUTION','up'=>$recorder->sql],JSON_PRETTY_PRINT),PHP_EOL;
    exit(0);
}
...
try {
    ...
    if (!$dsn || !str_starts_with($dsn, 'mysql:') || !str_contains($dsn, 'dbname=')) {
        throw new RuntimeException('Explicit WAVE4_DSN with database is required');
    }
    $pdo = new PDO($dsn, getenv('WAVE4_USER') ?: '', getenv('WAVE4_PASSWORD') ?: '', [
    ...
    foreach ($tableNames as $name) {
        ...
        foreach ($query('SELECT COLUMN_NAME,COLUMN_TYPE,IS_NULLABLE,COLUMN_DEFAULT,EXTRA,GENERATION_EXPRESSION,CHARACTER_SET_NAME,COLLATION_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=? AND TABLE_NAME=? ORDER BY ORDINAL_POSITION',[$schema,$name]) as $c) {
            ...
            $table['columns'][] = ['name'=>$c['COLUMN_NAME'],'type'=>$type,'nullable'=>$c['IS_NULLABLE']==='YES',
                ...
                'extra'=>$c['EXTRA'],'generation_expression'=>$c['GENERATION_EXPRESSION'],'auto_increment'=>str_contains($c['EXTRA'],'auto_increment'),'charset'=>$c['CHARACTER_SET_NAME'],'collation'=>$c['COLLATION_NAME']];
        }
        $indexes = [];
        foreach ($query('SELECT INDEX_NAME,COLUMN_NAME,NON_UNIQUE,SEQ_IN_INDEX,SUB_PART,INDEX_TYPE,COLLATION FROM information_schema.STATISTICS WHERE TABLE_SCHEMA=? AND TABLE_NAME=? ORDER BY INDEX_NAME,SEQ_IN_INDEX',[$schema,$name]) as $i) {
            if ($i['INDEX_NAME']==='PRIMARY') { $table['primary'][]=$i['COLUMN_NAME']; continue; }
            $indexes[$i['INDEX_NAME']]['name']=$i['INDEX_NAME'];$indexes[$i['INDEX_NAME']]['index_type']=$i['INDEX_TYPE'];$indexes[$i['INDEX_NAME']]['column_orders'][]=$i['COLLATION'];
    ...
    echo json_encode(['mode'=>'INFORMATION_SCHEMA','database_tables'=>array_column($query('SELECT TABLE_NAME FROM information_schema.TABLES WHERE TABLE_SCHEMA=?',[$schema]),'TABLE_NAME'),'engine'=>$version,'tables'=>$tables,'schema_sql'=>implode("\n\n",$snapshot)."\n"],JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES),PHP_EOL;
} catch (Throwable $e) {
    fwrite(STDERR, 'Wave 4 inspection blocked; error code: '.$e->getCode().'. No credentials or connection details emitted.'.PHP_EOL);
    exit(2);
}

...
def record(group, item):
    with LOCK:
        path = OUTPUT / f'wamp_{group}.json'
...
def run(group, label, suite, prefix, filter_text=None):
    ...
    schema = f"mepa_{'wave3' if prefix == 'WAVE3' else 'wave4'}_test_wamp_{run_id}"
    ...
    try:
        ...
        junit = OUTPUT / f'wamp_{group}_{label}_{run_id}.xml'
        ...
        log_file = OUTPUT / f'wamp_{group}_{label}_{run_id}.log'

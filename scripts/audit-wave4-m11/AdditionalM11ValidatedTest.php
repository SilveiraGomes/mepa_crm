...
final class AdditionalM11ValidatedTest extends WaveFourCase {
 ...
 public function test_progress_session_expires_during_audit_fk_wait():void{
  ...
  try{
   ...
   // Audit FK wait proves the decisive check already passed.$this->wait(fn()=>DomainClock::now($this->db())->format('Y-m-d H:i:s.u')>$expiry,'expiry',30);
  }finally{$lock->rollBack();}
  $out=stream_get_contents($pipes[1]);$err=stream_get_contents($pipes[2]);fclose($pipes[1]);fclose($pipes[2]);self::assertSame(0,proc_close($p),$out.$err);$r=json_decode($out,true,512,JSON_THROW_ON_ERROR);self::assertLessThan($expiry,$r['started']);$delta=$this->db()->table('discipleship_progress')->count()-$before;$ad=$this->db()->table('audit_logs')->count()-$audit;

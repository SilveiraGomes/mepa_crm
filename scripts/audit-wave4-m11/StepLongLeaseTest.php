...
final class StepLongLeaseTest extends WaveFourCase {
 ...
 private function race(string $method,bool $expire):void{
  ...
  try{
   ...
   $this->db()->statement('UPDATE auth_sessions SET expires_at=DATE_ADD(UTC_TIMESTAMP(6), INTERVAL '.($expire?'60':'120').' SECOND) WHERE id=?',[$f['auth']]);
   ...
   if($expire)$this->wait(fn()=>DomainClock::now($this->db())->format('Y-m-d H:i:s.u')>$expiry,'real expiry',90);

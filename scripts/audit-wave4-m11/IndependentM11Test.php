...
final class IndependentM11Test extends WaveFourCase {
 private array $evidence=[];
 private function save(string $name,array $v):void{$this->evidence[$name]=$v;file_put_contents(getenv('M11_OUT').'/independent.json',json_encode($this->evidence,JSON_PRETTY_PRINT));}
 ...
 private function race(string $method,bool $expire):void{
  ...
  try{
   ...
   self::assertLessThan($expiry,DomainClock::now($this->db())->format('Y-m-d H:i:s.u'),'session valid while worker actually blocked');
  ...
  self::assertSame($expire?'ACTOR_NOT_AUTHORIZED':'PASS',$r['result']);self::assertSame(0,$r['transaction_level']);

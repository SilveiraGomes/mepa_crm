...
try {
    ...
    try {
        if (($job['mode'] ?? 'checkout') === 'revoke') {
            $s->revokeAuthorization($f['actor'], $f['auth'], $job['authorization'] ?? $f['pickup'], 'SYNTHETIC_RACE_REASON');
            $result = ['result' => 'REVOKED'];
        } else {
            $other = !empty($job['other']);
            $result = $s->checkout($f['actor'], $f['auth'], $f['visit'], $other ? $f['other'] : $f['guardian'], $other ? $f['otherPickup'] : $f['pickup'], 'SYNTHETIC_IN_PERSON', true);
        }
        if (!empty($job['hold'])) {
            $db->commit();
        }
